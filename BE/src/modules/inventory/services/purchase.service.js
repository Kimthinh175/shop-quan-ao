...
class PurchaseService {
    async createPO(data) {
        ...
        if (poData.import_type === 'RI_LEVEL' && poData.ri_details && poData.ri_details.length > 0) {
            ...
            for (const ri of poData.ri_details) {
                ...
                if (variants.length > 0) {
                    const unit_cost = Math.round(ri.price_per_ri / variants.length);
        ...
        if (items && items.length > 0) {
            total = items.reduce((sum, item) => sum + (item.quantity * item.unit_cost), 0);
        }
        ...
        if (items && items.length > 0) {
            const poiPromises = items.map(item => PurchaseOrderItem.create({
                ...item,
    ...
    async getPOById(id) {
        ...
        if (!po) return null;
        const items = await PurchaseOrderItem.find({ purchase_order_id: id }).populate('product_variant_id');
        return {
            ...po.toObject(),
            items
        };
