...
export class ProductService {
  ...
  async fetchProducts(page: number = 1, limit: number = 12): Promise<import("../interfaces/ITypes").IPaginationResponse<import("../models/IProduct").IProduct>> {
    try {
      const response = await fetch(
        `${this.apiUrl}/products?page=${page}&limit=${limit}`,
      );
      if (!response.ok) return { results: this.mockProducts, limit: limit, totalResults: this.mockProducts.length, hasNextPage: false, hasPrevPage: false, nextCursor: null, prevCursor: null };

      const result = await response.json();
      return result;
    } catch (error) {
      console.error("Lỗi fetchProducts:", error);
      return { results: this.mockProducts, limit: limit, totalResults: this.mockProducts.length, hasNextPage: false, hasPrevPage: false, nextCursor: null, prevCursor: null };
    }
  }
